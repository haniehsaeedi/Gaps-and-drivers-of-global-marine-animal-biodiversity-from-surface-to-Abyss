...
# load data
load("Global_data_meso.rda") #to load the .rda

Global_data_meso <- mutate(Global_data_meso, dep_rnd_mes = floor(depth/10) * 10)

Global_data_meso <- Global_data_meso %>%
  filter(!is.na(decimalLongitude) & !is.na(decimalLatitude)) # remove NAs
Global_data_meso$scientificName <- stri_encode(Global_data_meso$scientificName, "", "UTF-8") # re-mark encodings

#Summary
summary(Global_data_meso)
Global_data_meso_table <- summary(Global_data_meso)
write.csv(Global_data_meso_table, 'Global_data_meso_table.csv')
...
total_counts_per_dep <- Global_data_meso %>%
  group_by(dep_rnd_mes) %>%
  summarise(total_scientific_names = n())
...
unique_counts_per_dep <- Global_data_meso %>%
  group_by(dep_rnd) %>%
  summarise(unique_scientific_names = n_distinct(scientificName))
# Calculate max, min, and mean of the unique scientific name counts
...
hist_data_lat <- hist(Global_data_meso$lat_5, breaks=seq(-90, 90, by=5), plot=FALSE)
...
# Create the plot
Lat_Num_Rec_Kernel_meso_col <- ggplot(Global_data_meso, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (660000 / max(after_stat(count))))), bw = 10, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 800000), breaks=seq(0, 800000, by=160000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Lat_Num_Rec_Kernel_meso_col.tiff", plot = Lat_Num_Rec_Kernel_meso_col, width = 7, height = 5, dpi = 600)

# Plot species numbers per latitude (with Kernel Estimation)
# Filter distinct values
distinct_data <- distinct(Global_data_meso, scientificName, lat_5, .keep_all = TRUE)
...
print(ad_test_1) # A = 0.30278, p-value = 0.5556

# Perform the Dip Test for unimodality
dip_test_1 <- dip.test(name_counts$distinct_names)
print(dip_test_1) # D = 0.06014, p-value = 0.4355

# Define axis limits for consistency
x_limits <- c(-90, 90) # Latitude range
y_limits <- c(0, 10000) # Fixed y-axis limits
...
# Create the plot
Lat_Num_Spe_Kernel_meso_col <- ggplot(distinct_data, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (7500 / max(after_stat(count))))), bw = 10, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 10000), breaks=seq(0, 10000, by=2000), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(-90, 90), breaks=seq(-90, 90, by=20), expand = c(0, 0)) +
...
ggsave("Lat_Num_Spe_Kernel_meso_col.tiff", plot = Lat_Num_Spe_Kernel_meso_col, width = 7, height = 5, dpi = 600)

...................................
# Plot records per depth (with Kernel Estimation)

# Step 1: Calculate the histogram
hist_data_dep <- hist(Global_data_meso$dep_rnd_mes, breaks=seq(200, 500, by=10), plot=FALSE)
...
print(ad_test_2) # A = 3.7262, p-value = 1.114e-09

# Perform the Dip Test for unimodality
dip_test_2 <- dip.test(hist_data_dep$counts)
print(dip_test_2) # D = 0.051924, p-value = 0.9566

# Define axis limits for consistency
x_limits <- c(200, 500) # Latitude range
y_limits <- c(0, 300000) # Fixed y-axis limits

# Calculate relative annotation positions
x_annotate <- x_limits[1] + 0.27 * diff(x_limits)  # 30% from the left
y_annotate_ad <- y_limits[1] + 0.92 * diff(y_limits)  # 95% up from the bottom
y_annotate_dip <- y_limits[1] + 0.82 * diff(y_limits)  # 85% up from the bottom

# Create the plot 
Dep_Num_Rec_Kernel_meso_col <- ggplot(Global_data_meso, aes(x = dep_rnd_mes)) +
  geom_histogram(binwidth = 10, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (224000 / max(after_stat(count))))), bw = 20,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 300000), breaks=seq(0, 300000, by=60000), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(200, 500), breaks=seq(200, 500, by=60), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Rec_Kernel_meso_col.tiff", plot = Dep_Num_Rec_Kernel_meso_col, width = 7, height = 5, dpi = 600)

.........................................
# Plot species per depth (with Kernel Estimation)
# Filter distinct values
distinct_data_2 <- distinct(Global_data_meso, scientificName, dep_rnd_mes, .keep_all = TRUE)

# Remove any non-finite values
distinct_data_2 <- distinct_data_2 %>% filter(is.finite(dep_rnd_mes))
...
print(ad_test_3) # A = 0.17971, p-value = 0.9085

# Perform the Dip Test for unimodality
dip_test_3 <- dip.test(name_counts_2$distinct_names_2)
print(dip_test_3) # D = 0.050825, p-value = 0.8016

# Define axis limits for consistency
x_limits <- c(200, 500) # Latitude range
y_limits <- c(0, 18000) # Fixed y-axis limits
...
#Create the plot
Dep_Num_Spe_Kernel_meso_col <- ggplot(distinct_data_2, aes(x = dep_rnd_mes)) +
  geom_histogram(binwidth = 10, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(density * nrow(distinct_data_2) * 12)), bw = 20,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits = c(0, 18000), breaks = seq(0, 18000, by = 3600), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits = c(200, 500), breaks = seq(200, 500, by=60), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Spe_Kernel_meso_col.tiff", plot = Dep_Num_Spe_Kernel_meso_col, width = 7, height = 5, dpi = 600)

.........................................
# Matrix Function
data <- Global_data_meso
data <- data %>%
  filter(!is.na(decimalLongitude) & !is.na(decimalLatitude)) # remove NAs
...
dep <- presence_absence_matrix(subset(data), "dep_rnd_mes", "scientificName")%>%  
  column_to_rownames(var = "dep_rnd_mes")
...
# Rarefaction
tiff("Rarefaction_meso.tiff", width = 6, height = 6, units = "in", res = 600)
rarecurve(lat, step = 20, sample = 50, col = "black", cex = 0.6, label = TRUE, main = "Meso")
dev.off()
...
print(ad_test_4) # A = 0.70509, p-value = 0.05969

# Perform the Dip Test for unimodality
dip_test_4 <- dip.test(lat_rarefied$.)
print(dip_test_4) # D = D = 0.053544, p-value = 0.6491
...
# Create the plot
Lat_ES50_Kernel_meso_col <- ggplot(lat_rarefied, aes(x = Latitude, y = .)) +
  geom_col(fill = "#1a80bb", color = "black", width = 5) +  # Set bar color to light grey
  geom_density(aes(y = after_stat(density * nrow(lat_rarefied) * 245)), bw = 10,
...
ggsave("Lat_ES50_Kernel_meso_col.tiff", plot = Lat_ES50_Kernel_meso_col, width = 7, height = 5, dpi = 600)
...
print(ad_test_5) # A = 0.3712, p-value = 0.4012

# Perform the Dip Test for unimodality
dip_test_5 <- dip.test(dep_rarefied$.)
print(dip_test_5) # D = 0.068307, p-value = 0.2915
...
# Create the plot
Dep_ES50_Kernel_meso_col <- ggplot(dep_rarefied, aes(x = Depth, y = .)) +
  geom_col(fill = "#1a80bb", color = "black", width = 10) +  # Set bar color to light grey
  geom_density(aes(y = after_stat(density * nrow(dep_rarefied) * 490)), bw = 20, # to create col_2
  #geom_density(data = dep_rarefied, aes(x = Depth, y = after_stat(density * 490 * nrow(dep_rarefied))), 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits = c(0, 70), breaks = seq(0, 70, by = 10), expand = c(0, 0)) +
  scale_x_continuous(limits = c(200, 500), breaks = seq(200, 500, by = 60), expand = c(0, 0)) +
...
ggsave("Dep_ES50_Kernel_meso_col.tiff", plot = Dep_ES50_Kernel_meso_col, width = 7, height = 5, dpi = 600)
...
library(writexl)
write_xlsx(spatial_data, path = "spatial_data_meso.xlsx")
...
breaks_Num_Records <- c(1, 100000, 300000, 500000, 700000, 809096)
labels_Num_Records <- c("1-100000", "100000-300000", "300000-500000", "500000-700000", "700000-809096")
...
breaks_Num_Species <- c(1, 500, 1000, 2000, 3000, 3809)
labels_Num_Species <- c("1-500", "500-1000", "1000-2000", "2000-3000", "3000-3809")
...
breaks_Chao1 <- c(1, 1000, 3000, 5000, 7000, 9453)
labels_Chao1 <- c("1-1000", "1000-3000", "3000-5000", "5000-7000", "7000-9453")
spatial_data$Chao1_Binned <- cut(spatial_data$Chao1, 
                                 breaks = breaks_Chao1, 
                                 labels = labels_Chao1, 
                                 include.lowest = TRUE)

breaks_ACE <- c(1, 1000, 2000, 3000, 4000, 5423)
labels_ACE <- c("1-1000", "1000-2000", "2000-3000", "3000-4000", "4000-5423")
spatial_data$ACE_Binned <- cut(spatial_data$ACE, 
                               breaks = breaks_ACE, 
                               labels = labels_ACE, 
                               include.lowest = TRUE)

breaks_Weighted  <- c(1, 100, 200, 300, 400, 533)
labels_Weighted  <- c("1-100", "100-200", "200-300", "300-400", "400-533")
spatial_data$Weighted_Binned <- cut(spatial_data$Weighted, 
                                    breaks = breaks_Weighted, 
                                    labels = labels_Weighted, 
                                    include.lowest = TRUE)

breaks_Chao2 <- c(1, 500, 1000, 2000, 3000, 3809)
labels_Chao2 <- c("1-500", "500-1000", "1000-2000", "2000-3000", "3000-3809")
...
abbreviated_labels_Num_Records <- c("1-100k", "100-300k", "300-500k", "500-700K", "700-809K")
...
ggsave("Num_Rec_Meso.tiff", plot = plot1, width = 6, height = 4, dpi = 600)
plot2 # Plot number of species
ggsave("Num_Spe_Meso.tiff", plot = plot2, width = 6, height = 4, dpi = 600)
plot3 # Plot ES50
ggsave("ES50_Meso.tiff", plot = plot3, width = 6, height = 4, dpi = 600)
plot4 # Plot Chao1
ggsave("Chao1_Meso.tiff", plot = plot4, width = 6, height = 4, dpi = 600)
plot5 # Plot ACE
ggsave("ACE_Meso.tiff", plot = plot5, width = 6, height = 4, dpi = 600)
plot6 # Plot Weighted
ggsave("Weighted_Meso.tiff", plot = plot6, width = 6, height = 4, dpi = 600)
plot7 # Plot Chao2
ggsave("Chao2_Meso.tiff", plot = plot7, width = 6, height = 4, dpi = 600)

save.image(file = "Meso_Analyses.RData")
load("Meso_Analyses.RData")
