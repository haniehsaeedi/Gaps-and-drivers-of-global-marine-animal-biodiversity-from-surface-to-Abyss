...
# load data
load("Global_data_Echinodermata.rda") #to load the .rda
...
total_counts_per_lat <- Global_data_Echinodermata %>%
  group_by(lat_5) %>%
...
unique_counts_per_lat <- Global_data_Echinodermata %>%
  group_by(lat_5) %>%
...
total_counts_per_dep <- Global_data_Echinodermata %>%
  group_by(dep_rnd) %>%
  summarise(total_scientific_names = n())
...
unique_counts_per_dep <- Global_data_Echinodermata %>%
  group_by(dep_rnd) %>%
  summarise(unique_scientific_names = n_distinct(scientificName))
...
hist_data_lat <- hist(Global_data_Echinodermata$lat_5, breaks=seq(-90, 90, by=5), plot=FALSE)
...
# Create the plot
Lat_Num_Rec_Kernel_Echinodermata_col <- ggplot(Global_data_Echinodermata, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (240000 / max(after_stat(count))))), bw = 10, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 320000), breaks=seq(0, 320000, by=64000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Lat_Num_Rec_Kernel_Echinodermata_col.tiff", plot = Lat_Num_Rec_Kernel_Echinodermata_col, width = 7, height = 5, dpi = 600)

# Plot species numbers per latitude (with Kernel Estimation)
# Filter distinct values
distinct_data <- distinct(Global_data_Echinodermata, scientificName, lat_5, .keep_all = TRUE)
...
print(ad_test_1) # A = 0.38186, p-value = 0.3804

# Perform the Dip Test for unimodality
dip_test_1 <- dip.test(name_counts$distinct_names)
print(dip_test_1) # D = 0.069098, p-value = 0.2152

# Define axis limits for consistency
x_limits <- c(-90, 90) # Latitude range
y_limits <- c(0, 3200) # Fixed y-axis limits
...
# Create the plot
Lat_Num_Spe_Kernel_Echinodermata_col <- ggplot(distinct_data, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (2500 / max(after_stat(count))))), bw = 10,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 3200), breaks=seq(0, 3200, by=640), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(-90, 90), breaks=seq(-90, 90, by=20), expand = c(0, 0)) +
...
ggsave("Lat_Num_Spe_Kernel_Echinodermata_col.tiff", plot = Lat_Num_Spe_Kernel_Echinodermata_col, width = 7, height = 5, dpi = 600)

...................................
# Plot records per depth (with Kernel Estimation)
# Step 1: Calculate the histogram
hist_data_dep <- hist(Global_data_Echinodermata$dep_rnd, breaks=seq(0, 11000, by=100), plot=FALSE)
...
print(ad_test_2) # A = 37.045, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_2 <- dip.test(hist_data_dep$counts)
print(dip_test_2) # D = 0.024655, p-value = 0.9458

# Define axis limits for consistency
x_limits <- c(0, 11000) # Latitude range
y_limits <- c(0, 220000) # Fixed y-axis limits
...
# Create the plot 
Dep_Num_Rec_Kernel_Echinodermata_col <- ggplot(Global_data_Echinodermata, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (160000 / max(after_stat(count))))), bw = 200,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 220000), breaks=seq(0, 220000, by=44000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Rec_Kernel_Echinodermata_col.tiff", plot = Dep_Num_Rec_Kernel_Echinodermata_col, width = 7, height = 5, dpi = 600)

.........................................
# Plot species per depth (with Kernel Estimation)
# Filter distinct values
distinct_data_2 <- distinct(Global_data_Echinodermata, scientificName, dep_rnd, .keep_all = TRUE)
...
print(ad_test_3) # A = 13.502, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_3 <- dip.test(name_counts_2$distinct_names_2)
print(dip_test_3) # D = 0.023223, p-value = 0.9909

# Define axis limits for consistency
x_limits <- c(0, 11000) # Latitude range
y_limits <- c(0, 5500) # Fixed y-axis limits
...
#Create the plot
Dep_Num_Spe_Kernel_Echinodermata_col <- ggplot(distinct_data_2, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a88bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(density * nrow(distinct_data_2) * 130)), bw = 200, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits = c(0, 5500), breaks = seq(0, 5500, by = 1100), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits = c(0, 11000), breaks = seq(0, 11000, by = 2000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Spe_Kernel_Echinodermata_col-t.tiff", plot = Dep_Num_Spe_Kernel_Echinodermata_col, width = 7, height = 5, dpi = 600)

.........................................
# Matrix Function
data <- Global_data_Echinodermata 
data <- data %>%
...
# Rarefaction
tiff("Rarefaction_Echinodermata.tiff", width = 6, height = 6, units = "in", res = 600)
rarecurve(lat, step = 20, sample = 50, col = "black", cex = 0.6, label = TRUE, main = "Echinodermata")
dev.off()
...
print(ad_test_4) # A = 1.6156, p-value = 0.0003035

# Perform the Dip Test for unimodality
dip_test_4 <- dip.test(lat_rarefied$.)
print(dip_test_4) # D = 0.044099, p-value = 0.912
...
# Create the plot
Lat_ES50_Kernel_Echinodermata_col <- ggplot(lat_rarefied, aes(x = Latitude, y = .)) +
...
ggsave("Lat_ES50_Kernel_Echinodermata_col.tiff", plot = Lat_ES50_Kernel_Echinodermata_col, width = 7, height = 5, dpi = 600)
...
print(ad_test_5) # A = 1.8742, p-value = 7.62e-05

# Perform the Dip Test for unimodality
dip_test_5 <- dip.test(dep_rarefied$.)
print(dip_test_5) # D = 0.047023, p-value = 0.4282
...
# Create the plot
Dep_ES50_Kernel_Echinodermata_col <- ggplot(dep_rarefied, aes(x = Depth, y = .)) +
...
ggsave("Dep_ES50_Kernel_Echinodermata_col-t.tiff", plot = Dep_ES50_Kernel_Echinodermata_col, width = 7, height = 5, dpi = 600)
...
breaks_Num_Records <- c(1, 50000, 100000, 150000, 200000, 209087)
labels_Num_Records <- c("1-50000", "50000-100000", "100000-150000", "150000-200000", "200000-209087")
...
breaks_Num_Species <- c(1, 200, 400, 600, 800, 1210)
labels_Num_Species <- c("1-200", "200-400", "400-600", "600-800", "800-1210")
...
breaks_Chao1 <- c(1, 200, 400, 600, 800, 1506.67)
labels_Chao1 <- c("1-200", "200-400", "400-600", "600-800", "800-1506")
spatial_data$Chao1_Binned <- cut(spatial_data$Chao1, 
                                 breaks = breaks_Chao1, 
                                 labels = labels_Chao1, 
                                 include.lowest = TRUE)

breaks_ACE <- c(1, 300, 600, 900, 1200, 1467.25)
labels_ACE <- c("1-300", "300-600", "600-900", "900-1200", "1200-1467")
spatial_data$ACE_Binned <- cut(spatial_data$ACE, 
                               breaks = breaks_ACE, 
                               labels = labels_ACE, 
                               include.lowest = TRUE)

breaks_Weighted  <- c(1, 50, 100, 150, 200, 267.45)
labels_Weighted  <- c("1-50", "50-100", "100-150", "150-200", "200-267")
spatial_data$Weighted_Binned <- cut(spatial_data$Weighted, 
                                    breaks = breaks_Weighted, 
                                    labels = labels_Weighted, 
                                    include.lowest = TRUE)

breaks_Chao2 <- c(1, 200, 400, 600, 800, 1210)
labels_Chao2 <- c("1-200", "200-400", "400-600", "600-800", "800-1210")
...
abbreviated_labels_Num_Records <- c("1-50k", "50-100k", "100-150k", "150-200K", "200-209K")
...
ggsave("Num_Rec_Echinodermata.tiff", plot = plot1, width = 6, height = 4, dpi = 600)
plot2 # Plot number of species
ggsave("Num_Spe_Echinodermata.tiff", plot = plot2, width = 6, height = 4, dpi = 600)
plot3 # Plot ES50
ggsave("ES50_Echinodermata.tiff", plot = plot3, width = 6, height = 4, dpi = 600)
plot4 # Plot Chao1
ggsave("Chao1_Echinodermata.tiff", plot = plot4, width = 6, height = 4, dpi = 600)
plot5 # Plot ACE
ggsave("ACE_Echinodermata.tiff", plot = plot5, width = 6, height = 4, dpi = 600)
plot6 # Plot Weighted
ggsave("Weighted_Echinodermata.tiff", plot = plot6, width = 6, height = 4, dpi = 600)
plot7 # Plot Chao2
ggsave("Chao2_Echinodermata.tiff", plot = plot7, width = 6, height = 4, dpi = 600)

save.image(file = "Echinodermata_Analyses.RData")
load("Echinodermata_Analyses.RData")