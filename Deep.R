...
# load data
load("Global_data_deep.rda") #to load the .rda

#Data formatting
Global_data_deep <- Global_data_deep %>%
  filter(!is.na(decimalLongitude) & !is.na(decimalLatitude)) # remove NAs
Global_data_deep$scientificName <- stri_encode(Global_data_deep$scientificName, "", "UTF-8") # re-mark encodings

#Summary
summary(Global_data_deep)
Global_data_deep_table <- summary(Global_data_deep)
write.csv(Global_data_deep_table, 'Global_data_deep_table.csv')
...
distinct_species_count <- subset(Global_data_deep, select = scientificName) %>% n_distinct()
print(distinct_species_count)

#Count the total number of scientific names per latitude bin
total_counts_per_lat <- Global_data_deep %>%
  group_by(lat_5) %>%
...
unique_counts_per_lat <- Global_data_deep %>%
  group_by(lat_5) %>%
...
total_counts_per_dep <- Global_data_deep %>%
  group_by(dep_rnd) %>%
  summarise(total_scientific_names = n())
...
unique_counts_per_dep <- Global_data_deep %>%
  group_by(dep_rnd) %>%
  summarise(unique_scientific_names = n_distinct(scientificName))
...
hist_data_lat <- hist(Global_data_deep$lat_5, breaks=seq(-90, 90, by=5), plot=FALSE)
...
# Create the plot
Lat_Num_Rec_Kernel_deep_col <- ggplot(Global_data_deep, aes(x = lat_5)) +
  ...
  scale_y_continuous(limits=c(0, 900000), breaks=seq(0, 900000, by=180000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Lat_Num_Rec_Kernel_deep_col.tiff", plot = Lat_Num_Rec_Kernel_deep_col, width = 7, height = 5, dpi = 600)

# Plot species numbers per latitude (with Kernel Estimation)
# Filter distinct values
distinct_data <- distinct(Global_data_deep, scientificName, lat_5, .keep_all = TRUE)
...
# Create the plot
Lat_Num_Spe_Kernel_deep_col <- ggplot(distinct_data, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
...
ggsave("Lat_Num_Spe_Kernel_deep_col.tiff", plot = Lat_Num_Spe_Kernel_deep_col, width = 7, height = 5, dpi = 600)

...................................
# Plot records per depth (with Kernel Estimation)
# Step 1: Calculate the histogram
hist_data_dep <- hist(Global_data_deep$dep_rnd, breaks=seq(500, 11000, by=100), plot=FALSE)
...
print(ad_test_2) # A = 23.959, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_2 <- dip.test(hist_data_dep$counts)
print(dip_test_2) # D = 0.026961, p-value = 0.8918

# Define axis limits for consistency
x_limits <- c(500, 11000) # Latitude range
...
# Create the plot 
Dep_Num_Rec_Kernel_deep_col <- ggplot(Global_data_deep, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (350000 / max(after_stat(count))))), bw = 100,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 500000), breaks=seq(0, 500000, by=100000), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(500, 11000), breaks=seq(500, 11000, by=2000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Rec_Kernel_deep_col.tiff", plot = Dep_Num_Rec_Kernel_deep_col, width = 7, height = 5, dpi = 600)

.........................................
# Plot species per depth (with Kernel Estimation)
# Filter distinct values
distinct_data_2 <- distinct(Global_data_deep, scientificName, dep_rnd, .keep_all = TRUE)
...
print(ad_test_3) # A = 15.281, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_3 <- dip.test(name_counts_2$distinct_names_2)
print(dip_test_3) # D = 0.037818, p-value = 0.3622

# Define axis limits for consistency
x_limits <- c(500, 11000) # Latitude range
y_limits <- c(0, 25000) # Fixed y-axis limits
...
#Create the plot
Dep_Num_Spe_Kernel_deep_col <- ggplot(distinct_data_2, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(density * nrow(distinct_data_2) * 130)), bw = 200,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits = c(0, 25000), breaks = seq(0, 25000, by = 5000), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits = c(500, 11000), breaks = seq(500, 11000, by=2000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Spe_Kernel_deep_col.tiff", plot = Dep_Num_Spe_Kernel_deep_col, width = 7, height = 5, dpi = 600)

.........................................
# Matrix Function
data <- Global_data_deep 
data <- data %>%
  filter(!is.na(decimalLongitude) & !is.na(decimalLatitude)) # remove NAs
...
# Rarefaction
tiff("Rarefaction_deep.tiff", width = 6, height = 6, units = "in", res = 600)
rarecurve(lat, step = 20, sample = 50, col = "black", cex = 0.6, label = TRUE, main = "Deep")
dev.off()
...
# Create the plot
Lat_ES50_Kernel_deep_col <- ggplot(lat_rarefied, aes(x = Latitude, y = .)) +
...
ggsave("Lat_ES50_Kernel_deep_col.tiff", plot = Lat_ES50_Kernel_deep_col, width = 7, height = 5, dpi = 600)
...
# Create the plot
Dep_ES50_Kernel_deep_col_2 <- ggplot(dep_rarefied, aes(x = Depth, y = .)) +
...
ggsave("Dep_ES50_Kernel_deep_col.tiff", plot = Dep_ES50_Kernel_deep_col, width = 7, height = 5, dpi = 600)
...
breaks_Num_Records <- c(1, 100000, 200000, 300000, 400000, 512758)
labels_Num_Records <- c("1-100000", "100000-200000", "200000-300000", "300000-400000", "400000-512758")
spatial_data$Num_Records_Binned <- cut(spatial_data$Num_Records, 
                                       breaks = breaks_Num_Records, 
                                       labels = labels_Num_Records, 
                                       include.lowest = TRUE)

breaks_Num_Species <- c(1, 1000, 2000, 3000, 4000, 5204)
labels_Num_Species <- c("1-1000", "1000-2000", "2000-3000", "3000-4000", "4000-5204")
...
breaks_Chao1 <- c(1, 1000, 2000, 3000, 4000, 6899)
labels_Chao1 <- c("1-1000", "1000-2000", "2000-3000", "3000-4000", "4000-6899")
spatial_data$Chao1_Binned <- cut(spatial_data$Chao1, 
                                breaks = breaks_Chao1, 
                                labels = labels_Chao1, 
                                include.lowest = TRUE)

breaks_ACE <- c(1, 1000, 2000, 3000, 4000, 7151)
labels_ACE <- c("1-1000", "1000-2000", "2000-3000", "3000-4000", "4000-7151")
spatial_data$ACE_Binned <- cut(spatial_data$ACE, 
                                 breaks = breaks_ACE, 
                                 labels = labels_ACE, 
                                 include.lowest = TRUE)

breaks_Weighted  <- c(1, 100, 200, 300, 400, 544)
labels_Weighted  <- c("1-100", "100-200", "200-300", "300-400", "400-544")
spatial_data$Weighted_Binned <- cut(spatial_data$Weighted, 
                               breaks = breaks_Weighted, 
                               labels = labels_Weighted, 
                               include.lowest = TRUE)

breaks_Chao2 <- c(1, 1000, 2000, 3000, 4000, 5204)
labels_Chao2 <- c("1-1000", "1000-2000", "2000-3000", "3000-4000", "4000-5204")
...
abbreviated_labels_Num_Records <- c("1-100k", "100-200k", "200-300k", "300-400K", "400-512K")
...
ggsave("Num_Rec_Deep.tiff", plot = plot1, width = 6, height = 4, dpi = 600)
plot2 # Plot number of species
ggsave("Num_Spe_Deep.tiff", plot = plot2, width = 6, height = 4, dpi = 600)
plot3 # Plot ES50
ggsave("ES50_Deep.tiff", plot = plot3, width = 6, height = 4, dpi = 600)
plot4 # Plot Chao1
ggsave("Chao1_Deep.tiff", plot = plot4, width = 6, height = 4, dpi = 600)
plot5 # Plot ACE
ggsave("ACE_Deep.tiff", plot = plot5, width = 6, height = 4, dpi = 600)
plot6 # Plot Weighted
ggsave("Weighted_Deep.tiff", plot = plot6, width = 6, height = 4, dpi = 600)
plot7 # Plot Chao2
ggsave("Chao2_Deep.tiff", plot = plot7, width = 6, height = 4, dpi = 600)

save.image(file = "Deep_Analyses.RData")
load("Deep_Analyses.RData")