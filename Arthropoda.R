...
# load data
load("Global_data_Arthropoda.rda") #to load the .rda
...
total_counts_per_lat <- Global_data_Arthropoda %>%
  group_by(lat_5) %>%
...
unique_counts_per_lat <- Global_data_Arthropoda %>%
  group_by(lat_5) %>%
...
total_counts_per_dep <- Global_data_Arthropoda %>%
  group_by(dep_rnd) %>%
  summarise(total_scientific_names = n())
...
unique_counts_per_dep <- Global_data_Arthropoda %>%
  group_by(dep_rnd) %>%
  summarise(unique_scientific_names = n_distinct(scientificName))
...
hist_data_lat <- hist(Global_data_Arthropoda$lat_5, breaks=seq(-90, 90, by=5), plot=FALSE)

# Step 2: Find the maximum count
# Calculate and print max, mean, and min in one line without storing
...
# Create the plot
Lat_Num_Rec_Kernel_Arthropoda_col <- ggplot(Global_data_Arthropoda, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (1200000 / max(after_stat(count))))), bw = 10, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 1500000), breaks=seq(0, 1500000, by=300000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Lat_Num_Rec_Kernel_Arthropoda_col.tiff", plot = Lat_Num_Rec_Kernel_Arthropoda_col, width = 7, height = 5, dpi = 600)

# Plot species numbers per latitude (with Kernel Estimation)
# Filter distinct values
distinct_data <- distinct(Global_data_Arthropoda, scientificName, lat_5, .keep_all = TRUE)
...
print(ad_test_1) # A = 0.41446, p-value = 0.3176

# Perform the Dip Test for unimodality
dip_test_1 <- dip.test(name_counts$distinct_names)
print(dip_test_1) # D = 0.069088, p-value = 0.2154

# Define axis limits for consistency
x_limits <- c(-90, 90) # Latitude range
y_limits <- c(0, 11000) # Fixed y-axis limits
...
# Create the plot
Lat_Num_Spe_Kernel_Arthropoda_col <- ggplot(distinct_data, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (8000 / max(after_stat(count))))), bw = 10,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 11000), breaks=seq(0, 11000, by=2200), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(-90, 90), breaks=seq(-90, 90, by=20), expand = c(0, 0)) +
...
ggsave("Lat_Num_Spe_Kernel_Arthropoda_col.tiff", plot = Lat_Num_Spe_Kernel_Arthropoda_col, width = 7, height = 5, dpi = 600)

...................................
# Plot records per depth (with Kernel Estimation)
# Step 1: Calculate the histogram
hist_data_dep <- hist(Global_data_Arthropoda$dep_rnd, breaks=seq(0, 11000, by=100), plot=FALSE)
...
print(ad_test_2) # A = 40.252, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_2 <- dip.test(hist_data_dep$counts)
print(dip_test_2) # D = 0.024563, p-value = 0.9485

# Define axis limits for consistency
x_limits <- c(0, 11000) # Latitude range
y_limits <- c(0, 1100000) # Fixed y-axis limits
...
# Create the plot 
Dep_Num_Rec_Kernel_Arthropoda_col <- ggplot(Global_data_Arthropoda, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (800000 / max(after_stat(count))))), bw = 140,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 1100000), breaks=seq(0, 1100000, by=220000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Rec_Kernel_Arthropoda_col_t.tiff", plot = Dep_Num_Rec_Kernel_Arthropoda_col, width = 7, height = 5, dpi = 600)

.........................................
# Plot species per depth (with Kernel Estimation)
# Filter distinct values
distinct_data_2 <- distinct(Global_data_Arthropoda, scientificName, dep_rnd, .keep_all = TRUE)
...
print(ad_test_3) # A = 19.612, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_3 <- dip.test(name_counts_2$distinct_names_2)
print(dip_test_3) # D = 0.035527, p-value = 0.4884
...
#Create the plot
Dep_Num_Spe_Kernel_Arthropoda_col <- ggplot(distinct_data_2, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a88bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(density * nrow(distinct_data_2) * 130)), bw = 220, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
...
ggsave("Dep_Num_Spe_Kernel_Arthropoda_col.tiff", plot = Dep_Num_Spe_Kernel_Arthropoda_col, width = 7, height = 5, dpi = 600)

.........................................
# Matrix Function
data <- Global_data_Arthropoda 
data <- data %>%
...
# Rarefaction
tiff("Rarefaction_Arthropoda.tiff", width = 6, height = 6, units = "in", res = 600)
rarecurve(lat, step = 20, sample = 50, col = "black", cex = 0.6, label = TRUE, main = "Arthropoda")
dev.off()
...
print(ad_test_4) # A = 0.41693, p-value = 0.3133

# Perform the Dip Test for unimodality
dip_test_4 <- dip.test(lat_rarefied$.)
print(dip_test_4) # D = 0.054536, p-value = 0.6159
...
# Create the plot
Lat_ES50_Kernel_Arthropoda_col <- ggplot(lat_rarefied, aes(x = Latitude, y = .)) +
  geom_col(fill = "#1a80bb", color = "black", width = 5) +  # Set bar color to light grey
  geom_density(aes(y = after_stat(density * nrow(lat_rarefied) * 240)), bw = 20,
...
ggsave("Lat_ES50_Kernel_Arthropoda_col-t.tiff", plot = Lat_ES50_Kernel_Arthropoda_col, width = 7, height = 5, dpi = 600)
...
print(ad_test_5) # A = 7.3286, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_5 <- dip.test(dep_rarefied$.)
print(dip_test_5) # D = 0.025424, p-value = 0.9931
...
# Create the plot
Dep_ES50_Kernel_Arthropoda_col <- ggplot(dep_rarefied, aes(x = Depth, y = .)) +
  geom_col(fill = "#1a80bb", color = "black", width = 100) +  # Set bar color to light grey
  geom_density(aes(y = after_stat(density * nrow(dep_rarefied) * 4800)), bw = 300, # to create col_2
...
ggsave("Dep_ES50_Kernel_Arthropoda_col.tiff", plot = Dep_ES50_Kernel_Arthropoda_col, width = 7, height = 5, dpi = 600)
...
breaks_Num_Records <- c(1, 100000, 200000, 400000, 600000, 756346)
labels_Num_Records <- c("1-100000", "100000-200000", "200000-400000", "400000-600000", "600000-756346")
...
breaks_Num_Species <- c(1, 500, 1000, 1500, 2000, 3168)
labels_Num_Species <- c("1-500", "500-1000", "1000-1500", "1500-2000", "2000-3168")
...
breaks_Chao1 <- c(1, 1000, 1500, 2000, 3000, 3969)
labels_Chao1 <- c("1-1000", "1000-1500", "1500-2000", "2000-3000", "3000-3969")
spatial_data$Chao1_Binned <- cut(spatial_data$Chao1, 
                                 breaks = breaks_Chao1, 
                                 labels = labels_Chao1, 
                                 include.lowest = TRUE)

breaks_ACE <- c(1, 1000, 1500, 2500, 3500, 4490)
labels_ACE <- c("1-1000", "1000-1500", "1500-2500", "2500-3500", "3500-4490")
spatial_data$ACE_Binned <- cut(spatial_data$ACE, 
                               breaks = breaks_ACE, 
                               labels = labels_ACE, 
                               include.lowest = TRUE)

breaks_Weighted  <- c(1, 100, 150, 200, 250, 391)
labels_Weighted  <- c("1-100", "100-150", "150-200", "200-250", "250-391")
spatial_data$Weighted_Binned <- cut(spatial_data$Weighted, 
                                    breaks = breaks_Weighted, 
                                    labels = labels_Weighted, 
                                    include.lowest = TRUE)

breaks_Chao2 <- c(1, 500, 1000, 2000, 3000, 3168)
labels_Chao2 <- c("1-500", "500-1000", "1000-2000", "2000-3000", "3000-3168")
...
abbreviated_labels_Num_Records <- c("1-100k", "100-200k", "200-400k", "400-600K", "600-756K")
...
ggsave("Num_Rec_Arthropoda.tiff", plot = plot1, width = 6, height = 4, dpi = 600)
plot2 # Plot number of species
ggsave("Num_Spe_Arthropoda.tiff", plot = plot2, width = 6, height = 4, dpi = 600)
plot3 # Plot ES50
ggsave("ES50_Arthropoda.tiff", plot = plot3, width = 6, height = 4, dpi = 600)
plot4 # Plot Chao1
ggsave("Chao1_Arthropoda.tiff", plot = plot4, width = 6, height = 4, dpi = 600)
plot5 # Plot ACE
ggsave("ACE_Arthropoda.tiff", plot = plot5, width = 6, height = 4, dpi = 600)
plot6 # Plot Weighted
ggsave("Weighted_Arthropoda.tiff", plot = plot6, width = 6, height = 4, dpi = 600)
plot7 # Plot Chao2
ggsave("Chao2_Arthropoda.tiff", plot = plot7, width = 6, height = 4, dpi = 600)

save.image(file = "Arthropoda_Analyses.RData")
load("Arthropoda_Analyses.RData")
