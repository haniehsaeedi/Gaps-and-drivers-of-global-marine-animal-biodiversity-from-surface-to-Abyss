...
# load data
load("Global_data_Chordata.rda") #to load the .rda
...
total_counts_per_lat <- Global_data_Chordata %>%
  group_by(lat_5) %>%
...
unique_counts_per_lat <- Global_data_Chordata %>%
  group_by(lat_5) %>%
...
total_counts_per_dep <- Global_data_Chordata %>%
  group_by(dep_rnd) %>%
  summarise(total_scientific_names = n())
...
unique_counts_per_dep <- Global_data_Chordata %>%
  group_by(dep_rnd) %>%
  summarise(unique_scientific_names = n_distinct(scientificName))
...
hist_data_lat <- hist(Global_data_Chordata$lat_5, breaks=seq(-90, 90, by=5), plot=FALSE)
...
# Create the plot
Lat_Num_Rec_Kernel_Chordata_col <- ggplot(Global_data_Chordata, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (4200000 / max(after_stat(count))))), bw = 10, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 5500000), breaks=seq(0, 5500000, by=1100000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Lat_Num_Rec_Kernel_Chordata_col.tiff", plot = Lat_Num_Rec_Kernel_Chordata_col, width = 7, height = 5, dpi = 600)

# Plot species numbers per latitude (with Kernel Estimation)
# Filter distinct values
distinct_data <- distinct(Global_data_Chordata, scientificName, lat_5, .keep_all = TRUE)
...
# Step 2: Find the maximum count
print(c(max_name_counts_lat = max(name_counts$distinct_names),
        min_name_counts_lat = min(name_counts$distinct_names),
        mean_name_counts = mean(name_counts$distinct_names)))

# Perform the Anderson-Darling test
ad_test_1 <- ad.test(name_counts$distinct_names)
print(ad_test_1) # A = 1.2657, p-value = 0.002301

# Perform the Dip Test for unimodality
dip_test_1 <- dip.test(name_counts$distinct_names)
print(dip_test_1) # D = 0.065002, p-value = 0.3021

# Define axis limits for consistency
x_limits <- c(-90, 90) # Latitude range
y_limits <- c(0, 15000) # Fixed y-axis limits
...
# Create the plot
Lat_Num_Spe_Kernel_Chordata_col <- ggplot(distinct_data, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (11000 / max(after_stat(count))))), bw = 10,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 15000), breaks=seq(0, 15000, by=3000), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(-90, 90), breaks=seq(-90, 90, by=20), expand = c(0, 0)) +
...
ggsave("Lat_Num_Spe_Kernel_Chordata_col.tiff", plot = Lat_Num_Spe_Kernel_Chordata_col, width = 7, height = 5, dpi = 600)

...................................
# Plot records per depth (with Kernel Estimation)
# Step 1: Calculate the histogram
hist_data_dep <- hist(Global_data_Chordata$dep_rnd, breaks=seq(0, 11000, by=100), plot=FALSE)
...
print(ad_test_2) # A = 40.498, p-value < 2.2e-16
...
y_limits <- c(0, 4500000) # Fixed y-axis limits
...
# Create the plot 
Dep_Num_Rec_Kernel_Chordata_col <- ggplot(Global_data_Chordata, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (3300000 / max(after_stat(count))))), bw = 200,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 4500000), breaks=seq(0, 4500000, by=900000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Rec_Kernel_Chordata_col.tiff", plot = Dep_Num_Rec_Kernel_Chordata_col, width = 7, height = 5, dpi = 600)

.........................................
# Plot species per depth (with Kernel Estimation)
# Filter distinct values
distinct_data_2 <- distinct(Global_data_Chordata, scientificName, dep_rnd, .keep_all = TRUE)
...
print(ad_test_3) # A = 18.942, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_3 <- dip.test(name_counts_2$distinct_names_2)
print(dip_test_3) # D = 0.041667, p-value = 0.3202

# Define axis limits for consistency
x_limits <- c(0, 11000) # Latitude range
y_limits <- c(0, 14000) # Fixed y-axis limits
...
#Create the plot
Dep_Num_Spe_Kernel_Chordata_col <- ggplot(distinct_data_2, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a88bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(density * nrow(distinct_data_2) * 110)), bw = 200, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits = c(0, 14000), breaks = seq(0, 14000, by = 2800), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits = c(0, 11000), breaks = seq(0, 11000, by = 2000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Spe_Kernel_Chordata_col.tiff", plot = Dep_Num_Spe_Kernel_Chordata_col, width = 7, height = 5, dpi = 600)

.........................................
# Matrix Function
data <- Global_data_Chordata 
# Select only the relevant columns to reduce processing time
data <- data %>%
  #select(scientificName, decimalLatitude, decimalLongitude) %>%  # Keep only the three columns for diversity analyses
...
# Rarefaction
tiff("Rarefaction_Chordata.tiff", width = 6, height = 6, units = "in", res = 600)
rarecurve(lat, step = 20, sample = 50, col = "black", cex = 0.6, label = TRUE, main = "Chordata")
dev.off()
...
print(ad_test_4) # A = 0.64016, p-value = 0.0871

# Perform the Dip Test for unimodality
dip_test_4 <- dip.test(lat_rarefied$.)
print(dip_test_4) # D = 0.037844, p-value = 0.9873
...
# Create the plot
Lat_ES50_Kernel_Chordata_col <- ggplot(lat_rarefied, aes(x = Latitude, y = .)) +
  geom_col(fill = "#1a80bb", color = "black", width = 5) +  # Set bar color to light grey
  geom_density(aes(y = after_stat(density * nrow(lat_rarefied) * 240)), bw = 30,
...
ggsave("Lat_ES50_Kernel_Chordata_col.tiff", plot = Lat_ES50_Kernel_Chordata_col, width = 7, height = 5, dpi = 600)
...
print(ad_test_5) # A = 1.2013, p-value = 0.003582

# Perform the Dip Test for unimodality
dip_test_5 <- dip.test(dep_rarefied$.)
print(dip_test_5) # D = 0.030975, p-value = 0.9794
...
# Create the plot
Dep_ES50_Kernel_Chordata_col <- ggplot(dep_rarefied, aes(x = Depth, y = .)) +
  geom_col(fill = "#1a80bb", color = "black", width = 100) +  # Set bar color to light grey
  geom_density(aes(y = after_stat(density * nrow(dep_rarefied) * 4800)), bw = 150, # to create col_2
...
ggsave("Dep_ES50_Kernel_Chordata_col.tiff", plot = Dep_ES50_Kernel_Chordata_col, width = 7, height = 5, dpi = 600)
...
breaks_Num_Records <- c(1, 500000, 1000000, 2000000, 3000000, 3275865)
labels_Num_Records <- c("1-500000", "500000-1000000", "1000000-2000000", "2000000-3000000", "3000000-3275865")
...
breaks_Num_Species <- c(1, 1000, 2000, 3000, 4000, 5109)
labels_Num_Species <- c("1-1000", "1000-2000", "2000-3000", "3000-4000", "4000-5109")
...
breaks_Chao1 <- c(1, 1000, 2000, 3000, 4000, 5583.1)
labels_Chao1 <- c("1-1000", "1000-2000", "2000-3000", "3000-4000", "4000-5583")
spatial_data$Chao1_Binned <- cut(spatial_data$Chao1, 
                                 breaks = breaks_Chao1, 
                                 labels = labels_Chao1, 
                                 include.lowest = TRUE)

breaks_ACE <- c(1, 1000, 2000, 3000, 4000, 5648.75)
labels_ACE <- c("1-1000", "1000-2000", "2000-3000", "3000-4000", "4000-5648")
spatial_data$ACE_Binned <- cut(spatial_data$ACE, 
                               breaks = breaks_ACE, 
                               labels = labels_ACE, 
                               include.lowest = TRUE)

breaks_Weighted  <- c(1, 200, 400, 600, 800, 986.120)
labels_Weighted  <- c("1-200", "200-400", "400-600", "600-800", "800-986")
spatial_data$Weighted_Binned <- cut(spatial_data$Weighted, 
                                    breaks = breaks_Weighted, 
                                    labels = labels_Weighted, 
                                    include.lowest = TRUE)

breaks_Chao2 <- c(1, 1000, 2000, 3000, 4000, 5109)
labels_Chao2 <- c("1-1000", "1000-2000", "2000-3000", "3000-4000", "4000-5109")
...
abbreviated_labels_Num_Records <- c("1-500k", "500-1000k", "1000-2000k", "2000-3000K", "3000-3275K")
...
ggsave("Num_Rec_Chordata.tiff", plot = plot1, width = 6, height = 4, dpi = 600)
plot2 # Plot number of species
ggsave("Num_Spe_Chordata.tiff", plot = plot2, width = 6, height = 4, dpi = 600)
plot3 # Plot ES50
ggsave("ES50_Chordata.tiff", plot = plot3, width = 6, height = 4, dpi = 600)
plot4 # Plot Chao1
ggsave("Chao1_Chordata.tiff", plot = plot4, width = 6, height = 4, dpi = 600)
plot5 # Plot ACE
ggsave("ACE_Chordata.tiff", plot = plot5, width = 6, height = 4, dpi = 600)
plot6 # Plot Weighted
ggsave("Weighted_Chordata.tiff", plot = plot6, width = 6, height = 4, dpi = 600)
plot7 # Plot Chao2
ggsave("Chao2_Chordata.tiff", plot = plot7, width = 6, height = 4, dpi = 600)

save.image(file = "Chordata_Analyses.RData")
load("Chordata_Analyses.RData")