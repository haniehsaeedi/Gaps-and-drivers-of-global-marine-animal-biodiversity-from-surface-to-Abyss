...
# load data
load("Global_data_Mollusca.rda") #to load the .rda
...
total_counts_per_lat <- Global_data_Mollusca %>%
  group_by(lat_5) %>%
...
unique_counts_per_lat <- Global_data_Mollusca %>%
  group_by(lat_5) %>%
...
total_counts_per_dep <- Global_data_Mollusca %>%
  group_by(dep_rnd) %>%
  summarise(total_scientific_names = n())
...
unique_counts_per_dep <- Global_data_Mollusca %>%
  group_by(dep_rnd) %>%
  summarise(unique_scientific_names = n_distinct(scientificName))
...
hist_data_lat <- hist(Global_data_Mollusca$lat_5, breaks=seq(-90, 90, by=5), plot=FALSE)
...
# Create the plot
Lat_Num_Rec_Kernel_Mollusca_col <- ggplot(Global_data_Mollusca, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (750000 / max(after_stat(count))))), bw = 10, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 1000000), breaks=seq(0, 1000000, by=200000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Lat_Num_Rec_Kernel_Mollusca_col.tiff", plot = Lat_Num_Rec_Kernel_Mollusca_col, width = 7, height = 5, dpi = 600)

# Plot species numbers per latitude (with Kernel Estimation)
# Filter distinct values
distinct_data <- distinct(Global_data_Mollusca, scientificName, lat_5, .keep_all = TRUE)
...
print(ad_test_1) # A = 0.79862, p-value = 0.03464

# Perform the Dip Test for unimodality
dip_test_1 <- dip.test(name_counts$distinct_names)
print(dip_test_1) # D = 0.046396, p-value = 0.8595

# Define axis limits for consistency
x_limits <- c(-90, 90) # Latitude range
y_limits <- c(0, 12000) # Fixed y-axis limits
...
# Create the plot
Lat_Num_Spe_Kernel_Mollusca_col <- ggplot(distinct_data, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (9000 / max(after_stat(count))))), bw = 10,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 12000), breaks=seq(0, 12000, by=2400), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(-90, 90), breaks=seq(-90, 90, by=20), expand = c(0, 0)) +
...
ggsave("Lat_Num_Spe_Kernel_Mollusca_col.tiff", plot = Lat_Num_Spe_Kernel_Mollusca_col, width = 7, height = 5, dpi = 600)

...................................
# Plot records per depth (with Kernel Estimation)
# Step 1: Calculate the histogram
hist_data_dep <- hist(Global_data_Mollusca$dep_rnd, breaks=seq(0, 11000, by=100), plot=FALSE)
...
print(ad_test_2) # A = 39.46, p-value < 2.2e-16
...
y_limits <- c(0, 460000) # Fixed y-axis limits
...
# Create the plot 
Dep_Num_Rec_Kernel_Mollusca_col <- ggplot(Global_data_Mollusca, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (320000 / max(after_stat(count))))), bw = 200,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 460000), breaks=seq(0, 460000, by=92000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Rec_Kernel_Mollusca_col.tiff", plot = Dep_Num_Rec_Kernel_Mollusca_col, width = 7, height = 5, dpi = 600)

.........................................
# Plot species per depth (with Kernel Estimation)
# Filter distinct values
distinct_data_2 <- distinct(Global_data_Mollusca, scientificName, dep_rnd, .keep_all = TRUE)
...
print(ad_test_3) # A = 19.55, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_3 <- dip.test(name_counts_2$distinct_names_2)
print(dip_test_3) # D = 0.053642, p-value = 0.07604

# Define axis limits for consistency
x_limits <- c(0, 11000) # Latitude range
y_limits <- c(0, 16000) # Fixed y-axis limits
...
#Create the plot
Dep_Num_Spe_Kernel_Mollusca_col <- ggplot(distinct_data_2, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a88bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(density * nrow(distinct_data_2) * 120)), bw = 200, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits = c(0, 16000), breaks = seq(0, 16000, by = 3200), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits = c(0, 11000), breaks = seq(0, 11000, by = 2000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Spe_Kernel_Mollusca_col.tiff", plot = Dep_Num_Spe_Kernel_Mollusca_col, width = 7, height = 5, dpi = 600)

.........................................
# Matrix Function
data <- Global_data_Mollusca 
data <- data %>%
...
# Rarefaction
tiff("Rarefaction_Mollusca.tiff", width = 6, height = 6, units = "in", res = 600)
rarecurve(lat, step = 20, sample = 50, col = "black", cex = 0.6, label = TRUE, main = "Mollusca")
dev.off()
...
print(ad_test_4) # A = 0.80329, p-value = 0.03372

# Perform the Dip Test for unimodality
dip_test_4 <- dip.test(lat_rarefied$.)
print(dip_test_4) # D = 0.053501, p-value = 0.6506
...
# Create the plot
Lat_ES50_Kernel_Mollusca_col <- ggplot(lat_rarefied, aes(x = Latitude, y = .)) +
...
ggsave("Lat_ES50_Kernel_Mollusca_col.tiff", plot = Lat_ES50_Kernel_Mollusca_col, width = 7, height = 5, dpi = 600)
...
print(ad_test_5) # A = 7.9618, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_5 <- dip.test(dep_rarefied$.)
print(dip_test_5) # D = 0.033321, p-value = 0.9294
...
# Create the plot
Dep_ES50_Kernel_Mollusca_col <- ggplot(dep_rarefied, aes(x = Depth, y = .)) +
...
ggsave("Dep_ES50_Kernel_Mollusca_col.tiff", plot = Dep_ES50_Kernel_Mollusca_col, width = 7, height = 5, dpi = 600)
...
breaks_Num_Records <- c(1, 100000, 200000, 300000, 400000, 597170)
labels_Num_Records <- c("1-100000", "100000-200000", "200000-300000", "300000-400000", "400000-597170")
...
breaks_Num_Species <- c(1, 1000, 2000, 3000, 4000, 4275)
labels_Num_Species <- c("1-1000", "1000-2000", "2000-3000", "3000-4000", "4000-4275")
...
breaks_Chao1 <- c(1, 1000, 2000, 4000, 5000, 8020.6)
labels_Chao1 <- c("1-1000", "1000-2000", "2000-4000", "4000-5000", "5000-8020")
spatial_data$Chao1_Binned <- cut(spatial_data$Chao1, 
                                 breaks = breaks_Chao1, 
                                 labels = labels_Chao1, 
                                 include.lowest = TRUE)

breaks_ACE <- c(1, 1000, 2000, 4000, 5000, 12949.24)
labels_ACE <- c("1-1000", "1000-2000", "2000-4000", "4000-5000", "5000-12949")
spatial_data$ACE_Binned <- cut(spatial_data$ACE, 
                               breaks = breaks_ACE, 
                               labels = labels_ACE, 
                               include.lowest = TRUE)

breaks_Weighted  <- c(1, 200, 400, 600, 800, 1010)
labels_Weighted  <- c("1-200", "200-400", "400-600", "600-800", "800-1010")
spatial_data$Weighted_Binned <- cut(spatial_data$Weighted, 
                                    breaks = breaks_Weighted, 
                                    labels = labels_Weighted, 
                                    include.lowest = TRUE)

breaks_Chao2 <- c(1, 500, 1000, 2000, 3000, 4275)
labels_Chao2 <- c("1-500", "500-1000", "1000-2000", "2000-3000", "3000-4275")
...
abbreviated_labels_Num_Records <- c("1-100k", "100-200k", "200-300k", "300-400K", "400-597K")
...
ggsave("Num_Rec_Mollusca.tiff", plot = plot1, width = 6, height = 4, dpi = 600)
plot2 # Plot number of species
ggsave("Num_Spe_Mollusca.tiff", plot = plot2, width = 6, height = 4, dpi = 600)
plot3 # Plot ES50
ggsave("ES50_Mollusca.tiff", plot = plot3, width = 6, height = 4, dpi = 600)
plot4 # Plot Chao1
ggsave("Chao1_Mollusca.tiff", plot = plot4, width = 6, height = 4, dpi = 600)
plot5 # Plot ACE
ggsave("ACE_Mollusca.tiff", plot = plot5, width = 6, height = 4, dpi = 600)
plot6 # Plot Weighted
ggsave("Weighted_Mollusca.tiff", plot = plot6, width = 6, height = 4, dpi = 600)
plot7 # Plot Chao2
ggsave("Chao2_Mollusca.tiff", plot = plot7, width = 6, height = 4, dpi = 600)

save.image(file = "Mollusca_Analyses.RData")
load("Mollusca_Analyses.RData")
