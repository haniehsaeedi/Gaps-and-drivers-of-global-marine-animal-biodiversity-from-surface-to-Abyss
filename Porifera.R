...
# load data
load("Global_data_Porifera.rda") #to load the .rda
...
total_counts_per_lat <- Global_data_Porifera %>%
  group_by(lat_5) %>%
...
unique_counts_per_lat <- Global_data_Porifera %>%
  group_by(lat_5) %>%
...
total_counts_per_dep <- Global_data_Porifera %>%
  group_by(dep_rnd) %>%
  summarise(total_scientific_names = n())
...
unique_counts_per_dep <- Global_data_Porifera %>%
  group_by(dep_rnd) %>%
  summarise(unique_scientific_names = n_distinct(scientificName))
...
hist_data_lat <- hist(Global_data_Porifera$lat_5, breaks=seq(-90, 90, by=5), plot=FALSE)
...
# Create the plot
Lat_Num_Rec_Kernel_Porifera_col <- ggplot(Global_data_Porifera, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (175000 / max(after_stat(count))))), bw = 10, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 230000), breaks=seq(0, 230000, by=46000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Lat_Num_Rec_Kernel_Porifera_col.tiff", plot = Lat_Num_Rec_Kernel_Porifera_col, width = 7, height = 5, dpi = 600)

# Plot species numbers per latitude (with Kernel Estimation)
# Filter distinct values
distinct_data <- distinct(Global_data_Porifera, scientificName, lat_5, .keep_all = TRUE)
...
print(ad_test_1) # A = 0.45864, p-value = 0.2475

# Perform the Dip Test for unimodality
dip_test_1 <- dip.test(name_counts$distinct_names)
print(dip_test_1) # D = 0.043684, p-value = 0.9193

# Define axis limits for consistency
x_limits <- c(-90, 90) # Latitude range
y_limits <- c(0, 2600) # Fixed y-axis limits
...
# Create the plot
Lat_Num_Spe_Kernel_Porifera_col <- ggplot(distinct_data, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (2000 / max(after_stat(count))))), bw = 10,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 2600), breaks=seq(0, 2600, by=520), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(-90, 90), breaks=seq(-90, 90, by=20), expand = c(0, 0)) +
...
ggsave("Lat_Num_Spe_Kernel_Porifera_col.tiff", plot = Lat_Num_Spe_Kernel_Porifera_col, width = 7, height = 5, dpi = 600)

...................................
# Plot records per depth (with Kernel Estimation)
# Step 1: Calculate the histogram
hist_data_dep <- hist(Global_data_Porifera$dep_rnd, breaks=seq(0, 11000, by=100), plot=FALSE)
...
y_limits <- c(0, 170000) # Fixed y-axis limits
...
# Create the plot 
Dep_Num_Rec_Kernel_Porifera_col <- ggplot(Global_data_Porifera, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (120000 / max(after_stat(count))))), bw = 300,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 170000), breaks=seq(0, 170000, by=34000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Dep_Num_Rec_Kernel_Porifera_col.tiff", plot = Dep_Num_Rec_Kernel_Porifera_col, width = 7, height = 5, dpi = 600)

.........................................
# Plot species per depth (with Kernel Estimation)
# Filter distinct values
distinct_data_2 <- distinct(Global_data_Porifera, scientificName, dep_rnd, .keep_all = TRUE)
...
print(ad_test_3) # A = 12.704, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_3 <- dip.test(name_counts_2$distinct_names_2)
print(dip_test_3) # D = 0.033869, p-value = 0.8664
...
#Create the plot
Dep_Num_Spe_Kernel_Porifera_col <- ggplot(distinct_data_2, aes(x = dep_rnd)) +
...
ggsave("Dep_Num_Spe_Kernel_Porifera_col.tiff", plot = Dep_Num_Spe_Kernel_Porifera_col, width = 7, height = 5, dpi = 600)

.........................................
# Matrix Function
data <- Global_data_Porifera 
data <- data %>%
...
# Rarefaction
tiff("Rarefaction_Porifera.tiff", width = 6, height = 6, units = "in", res = 600)
rarecurve(lat, step = 20, sample = 50, col = "black", cex = 0.6, label = TRUE, main = "Porifera")
dev.off()
...
print(ad_test_4) # A = 0.3342, p-value = 0.4925

# Perform the Dip Test for unimodality
dip_test_4 <- dip.test(lat_rarefied$.)
print(dip_test_4) # D = 0.060767, p-value = 0.4163
...
# Create the plot
Lat_ES50_Kernel_Porifera_col <- ggplot(lat_rarefied, aes(x = Latitude, y = .)) +
  geom_col(fill = "#1a80bb", color = "black", width = 5) +  # Set bar color to light grey
  geom_density(aes(y = after_stat(density * nrow(lat_rarefied) * 220)), bw = 20,
...
ggsave("Lat_ES50_Kernel_Porifera_col.tiff", plot = Lat_ES50_Kernel_Porifera_col, width = 7, height = 5, dpi = 600)
...
print(ad_test_5) # A = 1.1329, p-value = 0.005257

# Perform the Dip Test for unimodality
dip_test_5 <- dip.test(dep_rarefied$.)
print(dip_test_5) # D = 0.032902, p-value = 0.9723
...
# Create the plot
Dep_ES50_Kernel_Porifera_col <- ggplot(dep_rarefied, aes(x = Depth, y = .)) +
  geom_col(fill = "#1a80bb", color = "black", width = 100) +  # Set bar color to light grey
  geom_density(aes(y = after_stat(density * nrow(dep_rarefied) * 3500)), bw = 300, # to create col_2
...
ggsave("Dep_ES50_Kernel_Porifera_col-t.tiff", plot = Dep_ES50_Kernel_Porifera_col, width = 7, height = 5, dpi = 600)
...
breaks_Num_Records <- c(1, 50000, 100000, 150000, 200000, 241214)
labels_Num_Records <- c("1-50000", "50000-100000", "100000-150000", "150000-200000", "200000-241214")
spatial_data$Num_Records_Binned <- cut(spatial_data$Num_Records, 
                                       breaks = breaks_Num_Records, 
                                       labels = labels_Num_Records, 
                                       include.lowest = TRUE)

breaks_Num_Species <- c(1, 200, 400, 600, 800, 1379)
labels_Num_Species <- c("1-200", "200-400", "400-600", "600-800", "800-1379")
...
breaks_Chao1 <- c(1, 200, 400, 600, 800, 1545.58)
labels_Chao1 <- c("1-200", "200-400", "400-600", "600-800", "800-1545")
spatial_data$Chao1_Binned <- cut(spatial_data$Chao1, 
                                 breaks = breaks_Chao1, 
                                 labels = labels_Chao1, 
                                 include.lowest = TRUE)

breaks_ACE <- c(1, 300, 600, 900, 1200, 1566.3)
labels_ACE <- c("1-300", "300-600", "600-900", "900-1200", "1200-1566")
spatial_data$ACE_Binned <- cut(spatial_data$ACE, 
                               breaks = breaks_ACE, 
                               labels = labels_ACE, 
                               include.lowest = TRUE)

breaks_Weighted  <- c(1, 50, 100, 150, 200, 226.206)
labels_Weighted  <- c("1-50", "50-100", "100-150", "150-200", "200-226")
spatial_data$Weighted_Binned <- cut(spatial_data$Weighted, 
                                    breaks = breaks_Weighted, 
                                    labels = labels_Weighted, 
                                    include.lowest = TRUE)

breaks_Chao2 <- c(1, 200, 400, 600, 800, 1379)
labels_Chao2 <- c("1-200", "200-400", "400-600", "600-800", "800-1379")
...
abbreviated_labels_Num_Records <- c("1-50k", "50-100k", "100-150k", "150-200K", "200-241K")
...
ggsave("Num_Rec_Porifera.tiff", plot = plot1, width = 6, height = 4, dpi = 600)
plot2 # Plot number of species
ggsave("Num_Spe_Porifera.tiff", plot = plot2, width = 6, height = 4, dpi = 600)
plot3 # Plot ES50
ggsave("ES50_Porifera.tiff", plot = plot3, width = 6, height = 4, dpi = 600)
plot4 # Plot Chao1
ggsave("Chao1_Porifera.tiff", plot = plot4, width = 6, height = 4, dpi = 600)
plot5 # Plot ACE
ggsave("ACE_Porifera.tiff", plot = plot5, width = 6, height = 4, dpi = 600)
plot6 # Plot Weighted
ggsave("Weighted_Porifera.tiff", plot = plot6, width = 6, height = 4, dpi = 600)
plot7 # Plot Chao2
ggsave("Chao2_Porifera.tiff", plot = plot7, width = 6, height = 4, dpi = 600)

save.image(file = "Porifera_Analyses.RData")
load("Porifera_Analyses.RData")
