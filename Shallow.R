...
# load data
load("Global_data_shallow.rda") #to load the .rda

#Data formatting
Global_data_shallow <- Global_data_shallow %>%
  filter(!is.na(decimalLongitude) & !is.na(decimalLatitude)) # remove NAs
Global_data_shallow$scientificName <- stri_encode(Global_data_shallow$scientificName, "", "UTF-8") # re-mark encodings

Global_data_shallow <- mutate(Global_data_shallow, dep_rnd_sha = floor(depth/10) * 10)

#Shallow
#Summary
summary(Global_data_shallow)
Global_data_shallow_table <- summary(Global_data_shallow)
write.csv(Global_data_shallow_table, 'Global_data_shallow_table.csv')
...
total_counts_per_lat <- Global_data_shallow %>%
  group_by(lat_5) %>%
...
unique_counts_per_lat <- Global_data_shallow %>%
  group_by(lat_5) %>%
...
total_counts_per_dep <- Global_data_shallow %>%
  group_by(dep_rnd_sha) %>%
  summarise(total_scientific_names = n())
...
unique_counts_per_dep <- Global_data_shallow %>%
  group_by(dep_rnd_sha) %>%
  summarise(unique_scientific_names = n_distinct(scientificName))
...
hist_data_lat <- hist(Global_data_shallow$lat_5, breaks=seq(-90, 90, by=5), plot=FALSE)
...
print(ad_test) # A = 4.2057, p-value = 1.118e-10
...
y_limits <- c(0, 9500000) # Fixed y-axis limits

# Calculate relative annotation positions
x_annotate <- x_limits[1] + 0.28 * diff(x_limits)  # 30% from the left
...
# Create the plot
Lat_Num_Rec_Kernel_shallow_col <- ggplot(Global_data_shallow, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (7500000 / max(after_stat(count))))), bw = 10,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 9500000), breaks=seq(0, 9500000, by=1900000), expand = c(0, 0), labels = scales::label_comma()) +
...
ggsave("Lat_Num_Rec_Kernel_shallow_col.tiff", plot = Lat_Num_Rec_Kernel_shallow_col, width = 7, height = 5, dpi = 600)

# Plot species numbers per latitude (with Kernel Estimation)
# Filter distinct values
distinct_data <- distinct(Global_data_shallow, scientificName, lat_5, .keep_all = TRUE)
...
print(ad_test_1) # A = 0.42718, p-value = 0.2958

# Perform the Dip Test for unimodality
dip_test_1 <- dip.test(name_counts$distinct_names)
print(dip_test_1) # D = 0.043133, p-value = 0.9291

# Define axis limits for consistency
x_limits <- c(-90, 90) # Latitude range
y_limits <- c(0, 30000) # Fixed y-axis limits
...
# Create the plot
Lat_Num_Spe_Kernel_shallow_col <- ggplot(distinct_data, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (26000 / max(after_stat(count))))), bw = 10,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 30000), breaks=seq(0, 30000, by=6000), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(-90, 90), breaks=seq(-90, 90, by=20), expand = c(0, 0)) +
...
ggsave("Lat_Num_Spe_Kernel_shallow_col.tiff", plot = Lat_Num_Spe_Kernel_shallow_col, width = 7, height = 5, dpi = 600)

...................................
# Plot records per depth (with Kernel Estimation)

# Step 1: Calculate the histogram
hist_data_dep <- hist(Global_data_shallow$dep_rnd_sha, breaks=seq(0, 200, by=10), plot=FALSE)
...
# Define axis limits for consistency
x_limits <- c(0, 200) # Latitude range
y_limits <- c(0, 9000000) # Fixed y-axis limits
...
# Create the plot 
Dep_Num_Rec_Kernel_shallow_col <- ggplot(Global_data_shallow, aes(x = dep_rnd_sha)) +
  geom_histogram(binwidth = 10, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (6500000 / max(after_stat(count))))), bw = 20, 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 9000000), breaks=seq(0, 9000000, by=1800000), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(0, 200), breaks=seq(0, 200, by=40), expand = c(0, 0)) +
...
ggsave("Dep_Num_Rec_Kernel_shallow_col_t.tiff", plot = Dep_Num_Rec_Kernel_shallow_col, width = 7, height = 5, dpi = 600)

.........................................
# Plot species per depth (with Kernel Estimation)
# Filter distinct values
distinct_data_2 <- distinct(Global_data_shallow, scientificName, dep_rnd_sha, .keep_all = TRUE)

# Remove any non-finite values
distinct_data_2 <- distinct_data_2 %>% filter(is.finite(dep_rnd_sha))
...
print(ad_test_3) # A = 1.0644, p-value = 0.006684

# Perform the Dip Test for unimodality
dip_test_3 <- dip.test(name_counts_2$distinct_names_2)
print(dip_test_3) # D = 0.052611, p-value = 0.9373

# Define axis limits for consistency
x_limits <- c(0, 200) # Latitude range
y_limits <- c(0, 74000) # Fixed y-axis limits
...
#Create the plot
Dep_Num_Spe_Kernel_shallow_col <- ggplot(distinct_data_2, aes(x = dep_rnd_sha)) +
  geom_histogram(binwidth = 10, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(density * nrow(distinct_data_2) * 12)), bw = 20,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits = c(0, 74000), breaks = seq(0, 74000, by = 14800), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits = c(0, 200), breaks = seq(0, 200, by=40), expand = c(0, 0)) +
...
ggsave("Dep_Num_Spe_Kernel_shallow_col.tiff", plot = Dep_Num_Spe_Kernel_shallow_col, width = 7, height = 5, dpi = 600)

.........................................
# Matrix Function
data <- Global_data_shallow
data <- data %>%
  filter(!is.na(decimalLongitude) & !is.na(decimalLatitude)) # remove NAs
...
dep <- presence_absence_matrix(subset(data), "dep_rnd_sha", "scientificName")%>%  
  column_to_rownames(var = "dep_rnd_sha")
...
# Rarefaction
tiff("Rarefaction_shallow.tiff", width = 6, height = 6, units = "in", res = 600)
rarecurve(lat, step = 20, sample = 50, col = "black", cex = 0.6, label = TRUE, main = "Shallow")
dev.off()
...
print(ad_test_4) # A = 1.0122, p-value = 0.01002

# Perform the Dip Test for unimodality
dip_test_4 <- dip.test(lat_rarefied$.)
print(dip_test_4) # D = 0.048985, p-value = 0.7939
...
# Create the plot
Lat_ES50_Kernel_shallow_col <- ggplot(lat_rarefied, aes(x = Latitude, y = .)) +
...
ggsave("Lat_ES50_Kernel_shallow_col.tiff", plot = Lat_ES50_Kernel_shallow_col_2, width = 7, height = 5, dpi = 600)
...
print(ad_test_5) # A = 0.42305, p-value = 0.2911

# Perform the Dip Test for unimodality
dip_test_5 <- dip.test(dep_rarefied$.)
print(dip_test_5) # D = 0.046062, p-value = 0.9902
...
# Create the plot
Dep_ES50_Kernel_shallow_col_2 <- ggplot(dep_rarefied, aes(x = Depth, y = .)) +
  geom_col(fill = "#1a80bb", color = "black", width = 10) +  # Set bar color to light grey
  geom_density(aes(y = after_stat(density * nrow(dep_rarefied) * 490)), bw = 10, # to create col_2
  #geom_density(data = dep_rarefied, aes(x = Depth, y = ..density.. * 480 * nrow(dep_rarefied)), 
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits = c(0, 70), breaks = seq(0, 70, by = 10), expand = c(0, 0)) +
  scale_x_continuous(limits = c(0, 200), breaks = seq(0, 200, by = 40), expand = c(0, 0)) +
...
ggsave("Dep_ES50_Kernel_shallow_col.tiff", plot = Dep_ES50_Kernel_shallow_col, width = 7, height = 5, dpi = 600)
...
breaks_Num_Records <- c(1, 100000, 1000000, 2000000, 4000000, 6258830)
labels_Num_Records <- c("1-100000", "100000-1000000", "1000000-2000000", "2000000-4000000", "4000000-6258830")
spatial_data$Num_Records_Binned <- cut(spatial_data$Num_Records, 
                                       breaks = breaks_Num_Records, 
                                       labels = labels_Num_Records, 
                                       include.lowest = TRUE)

breaks_Num_Species <- c(1, 1000, 3000, 6000, 9000, 11419)
labels_Num_Species <- c("1-1000", "1000-3000", "3000-6000", "6000-9000", "9000-11419")
...
breaks_Chao1 <- c(1, 3000, 6000, 9000, 12000, 13882.3)
labels_Chao1 <- c("1-3000", "3000-6000", "6000-9000", "9000-12000", "12000-13882")
spatial_data$Chao1_Binned <- cut(spatial_data$Chao1, 
                                 breaks = breaks_Chao1, 
                                 labels = labels_Chao1, 
                                 include.lowest = TRUE)

breaks_ACE <- c(1, 3000, 6000, 9000, 12000, 16530.276)
labels_ACE <- c("1-3000", "3000-6000", "6000-9000", "9000-12000", "12000-16530")
spatial_data$ACE_Binned <- cut(spatial_data$ACE, 
                               breaks = breaks_ACE, 
                               labels = labels_ACE, 
                               include.lowest = TRUE)

breaks_Weighted  <- c(1, 100, 200, 300, 400, 774.467)
labels_Weighted  <- c("1-100", "100-200", "200-300", "300-400", "400-774")
spatial_data$Weighted_Binned <- cut(spatial_data$Weighted, 
                                    breaks = breaks_Weighted, 
                                    labels = labels_Weighted, 
                                    include.lowest = TRUE)

breaks_Chao2 <- c(1, 2000, 4000, 6000, 8000, 11419)
labels_Chao2 <- c("1-2000", "2000-4000", "4000-6000", "6000-8000", "8000-11419")
...
abbreviated_labels_Num_Records <- c("1-100k", "100-1000k", "1000-2000k", "2000-4000K", "4000-6258K")
...
ggsave("Num_Rec_Shallow_rev.tiff", plot = plot1, width = 6, height = 4, dpi = 600)
plot2 # Plot number of species
ggsave("Num_Spe_Shallow_rev.tiff", plot = plot2, width = 6, height = 4, dpi = 600)
plot3 # Plot ES50
ggsave("ES50_Shallow.tiff", plot = plot3, width = 6, height = 4, dpi = 600)
plot4 # Plot Chao1
ggsave("Chao1_Deep.tiff", plot = plot4, width = 6, height = 4, dpi = 600)
plot5 # Plot ACE
ggsave("ACE_Shallow.tiff", plot = plot5, width = 6, height = 4, dpi = 600)
plot6 # Plot Weighted
ggsave("Weighted_Shallow.tiff", plot = plot6, width = 6, height = 4, dpi = 600)
plot7 # Plot Chao2
ggsave("Chao2_Shallow.tiff", plot = plot7, width = 6, height = 4, dpi = 600)

save.image(file = "Shallow_Analyses.RData")
load("Shallow_Analyses_2.RData")
