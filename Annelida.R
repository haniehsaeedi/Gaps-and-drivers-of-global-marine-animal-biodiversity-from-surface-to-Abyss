...
# load data
load("Global_data_Annelida.rda") #to load the .rda
...
total_counts_per_lat <- Global_data_Annelida %>%
  group_by(lat_5) %>%
...
unique_counts_per_lat <- Global_data_Annelida %>%
  group_by(lat_5) %>%
...
total_counts_per_dep <- Global_data_Annelida %>%
  group_by(dep_rnd) %>%
  summarise(total_scientific_names = n())
...
unique_counts_per_dep <- Global_data_Annelida %>%
  group_by(dep_rnd) %>%
  summarise(unique_scientific_names = n_distinct(scientificName))
...
hist_data_lat <- hist(Global_data_Annelida$lat_5, breaks=seq(-90, 90, by=5), plot=FALSE)
...
# Create the plot
Lat_Num_Rec_Kernel_Annellida_col <- ggplot(Global_data_Annelida, aes(x = lat_5)) +
...
ggsave("Lat_Num_Rec_Kernel_Annellida_col.tiff", plot = Lat_Num_Rec_Kernel_Annellida_col, width = 7, height = 5, dpi = 600)

# Plot species numbers per latitude (with Kernel Estimation)
# Filter distinct values
distinct_data <- distinct(Global_data_Annelida, scientificName, lat_5, .keep_all = TRUE)
...
print(ad_test_1) # A = 0.54898, p-value = 0.146

# Perform the Dip Test for unimodality
dip_test_1 <- dip.test(name_counts$distinct_names)
print(dip_test_1) # D = 0.046022, p-value = 0.8688

# Define axis limits for consistency
x_limits <- c(-90, 90) # Latitude range
y_limits <- c(0, 4000) # Fixed y-axis limits
...
# Create the plot
Lat_Num_Spe_Kernel_Annelida_col <- ggplot(distinct_data, aes(x = lat_5)) +
  geom_histogram(binwidth = 5, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (3200 / max(after_stat(count))))), bw = 10,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
  scale_y_continuous(limits=c(0, 4000), breaks=seq(0, 4000, by=800), expand = c(0, 0), labels = scales::label_comma()) +
  scale_x_continuous(limits=c(-90, 90), breaks=seq(-90, 90, by=20), expand = c(0, 0)) +
...
ggsave("Lat_Num_Spe_Kernel_Annelida_col.tiff", plot = Lat_Num_Spe_Kernel_Annelida_col, width = 7, height = 5, dpi = 600)

...................................
# Plot records per depth (with Kernel Estimation)
# Step 1: Calculate the histogram
hist_data_dep <- hist(Global_data_Annelida$dep_rnd, breaks=seq(0, 11000, by=100), plot=FALSE)
...
print(ad_test_2) # A = 39.297, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_2 <- dip.test(hist_data_dep$counts)
print(dip_test_2) # D = 0.040909, p-value = 0.1979
...
# Create the plot 
Dep_Num_Rec_Kernel_Annelida_col <- ggplot(Global_data_Annelida, aes(x = dep_rnd)) +
  geom_histogram(binwidth = 100, fill = "#1a80bb", color = "black", position = "identity") +
  geom_density(aes(y = after_stat(count * (600000 / max(after_stat(count))))), bw = 200,
               alpha = 0.5, fill = "NA", color = "#ea801c", size = 1.5) +
...
ggsave("Dep_Num_Rec_Kernel_Annelida_col.tiff", plot = Dep_Num_Rec_Kernel_Annelida_col, width = 7, height = 5, dpi = 600)

.........................................
# Add kernel density
# Filter distinct values
distinct_data_2 <- distinct(Global_data_Annelida, scientificName, dep_rnd, .keep_all = TRUE)
...
print(ad_test_3) # A = 18.594, p-value < 2.2e-16

# Perform the Dip Test for unimodality
dip_test_3 <- dip.test(name_counts_2$distinct_names_2)
print(dip_test_3) # D = 0.047872, p-value = 0.1133
...
#Create the plot
Dep_Num_Spe_Kernel_Annelida_col <- ggplot(distinct_data_2, aes(x = dep_rnd)) +
...
ggsave("Dep_Num_Spe_Kernel_Annelida_col.tiff", plot = Dep_Num_Spe_Kernel_Annelida_col, width = 7, height = 5, dpi = 600)

.........................................
# Matrix Function
data <- Global_data_Annelida 
data <- data %>%
...
# Rarefaction
tiff("Rarefaction_Annelida.tiff", width = 6, height = 6, units = "in", res = 600)
rarecurve(lat, step = 20, sample = 50, col = "black", cex = 0.6, label = TRUE, main = "Annelida")
dev.off()
...
print(ad_test_4) # A = 2.1733, p-value = 1.214e-05

# Perform the Dip Test for unimodality
dip_test_4 <- dip.test(lat_rarefied$.)
print(dip_test_4) # D = 0.041946, p-value = 0.9502
...
# Create the plot
Lat_ES50_Kernel_Annelida_col <- ggplot(lat_rarefied, aes(x = Latitude, y = .)) +
...
ggsave("Lat_ES50_Kernel_Annelida_col_t.tiff", plot = Lat_ES50_Kernel_Annelida_col, width = 7, height = 5, dpi = 600)
...
print(ad_test_5) # A = 4.6711, p-value = 1.009e-11

# Perform the Dip Test for unimodality
dip_test_5 <- dip.test(dep_rarefied$.)
print(dip_test_5) # D = 0.03765, p-value = 0.8207
...
# Create the plot
Dep_ES50_Kernel_Annelida_col <- ggplot(dep_rarefied, aes(x = Depth, y = .)) +
  geom_col(fill = "#1a80bb", color = "black", width = 100) +  # Set bar color to light grey
  geom_density(aes(y = after_stat(density * nrow(dep_rarefied) * 4600)), bw = 300, # to create col_2
...
ggsave("Dep_ES50_Kernel_Annelida_col_t.tiff", plot = Dep_ES50_Kernel_Annelida_col, width = 7, height = 5, dpi = 600)
...
breaks_Num_Records <- c(1, 100000, 300000, 500000, 700000, 1143156)
labels_Num_Records <- c("1-100000", "100000-300000", "300000-500000", "500000-700000", "700000-1143156")
...
breaks_Num_Species <- c(1, 300, 600, 900 , 1200, 1631)
labels_Num_Species <- c("1-300", "300-600", "600-900", "900-1200", "1200-1631")
...
breaks_Chao1 <- c(1, 300, 600, 900, 1200, 1864)
labels_Chao1 <- c("1-300", "300-600", "600-900", "900-1200", "1200-1864")
spatial_data$Chao1_Binned <- cut(spatial_data$Chao1, 
                                 breaks = breaks_Chao1, 
                                 labels = labels_Chao1, 
                                 include.lowest = TRUE)

breaks_ACE <- c(1, 300, 600, 900, 1200, 1848)
labels_ACE <- c("1-300", "300-600", "600-900", "900-1200", "1200-1848")
spatial_data$ACE_Binned <- cut(spatial_data$ACE, 
                               breaks = breaks_ACE, 
                               labels = labels_ACE, 
                               include.lowest = TRUE)

breaks_Weighted  <- c(1, 100, 150, 200, 250, 322)
labels_Weighted  <- c("1-100", "100-150", "150-200", "200-250", "250-322")
spatial_data$Weighted_Binned <- cut(spatial_data$Weighted, 
                                    breaks = breaks_Weighted, 
                                    labels = labels_Weighted, 
                                    include.lowest = TRUE)

breaks_Chao2 <- c(1, 300, 600, 900, 1200, 1631)
labels_Chao2 <- c("1-300", "300-600", "600-900", "900-1200", "1200-1631")
...
abbreviated_labels_Num_Records <- c("1-100k", "100-300k", "300-500k", "500-700K", "700-1143K")
...
ggsave("Num_Rec_Annelida.tiff", plot = plot1, width = 6, height = 4, dpi = 600)
plot2 # Plot number of species
ggsave("Num_Spe_Annelida.tiff", plot = plot2, width = 6, height = 4, dpi = 600)
plot3 # Plot ES50
ggsave("ES50_Annelida.tiff", plot = plot3, width = 6, height = 4, dpi = 600)
plot4 # Plot Chao1
ggsave("Chao1_Annelida.tiff", plot = plot4, width = 6, height = 4, dpi = 600)
plot5 # Plot ACE
ggsave("ACE_Annelida.tiff", plot = plot5, width = 6, height = 4, dpi = 600)
plot6 # Plot Weighted
ggsave("Weighted_Annelida.tiff", plot = plot6, width = 6, height = 4, dpi = 600)
plot7 # Plot Chao2
ggsave("Chao2_Annelida.tiff", plot = plot7, width = 6, height = 4, dpi = 600)

save.image(file = "Annelida_Analyses.RData")
load("Annelida_Analyses.RData")